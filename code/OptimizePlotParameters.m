...
for n=3:1:3
    ...
    model = stvm_train(full(trainX),full(trainY),full(testX),options);
    [erate, nvec, label, y_prob] = stvm_predict(full(testY),model);
    theta_initial	= 1;
    
    
    fprintf('\nSTVM CLASSIFICATION test error: %.2f%%', erate*100)
    fprintf('\nNumber of vectors : %d \n \n',nvec)
    ...
    title("STVM, Number Vectors = "+num2str(nvec)+", Error = "+num2str(round(erate*100))+"%" ,'FontSize',14)
end
